...
#+NAME: hold-flip
| BOOT  | DF(U_TAP) | OSL_U_SYM | DF(U_BASE) | SCLN  |         | DF(U_BASE) | DF(U_EXTRA) | DF(U_TAP) | BOOT     |
| LGUI  | LALT      | LCTL      | LSFT       | COLN  |         | LSFT       | LCTL        | LALT      | LGUI     |
| U_UND | U_CUT     | U_CPY     | U_PST      | U_RDO |         | LOCK_CUR   | LOCK_OPP    | ALGR      | U_BUTTON |
...
#+NAME: fun-r
| REC_START | F7  | F8   | F9   | F12  |
| REC_PLAY  | F1  | F2   | F3   | F10  |
...
| WH_R        | WH_R            | U_WH_R       |               |
| WH_U        | WH_U            | U_WH_U       |               |
| REC_START   | DYN_REC_START1  |              |               |
| REC_PLAY    | DYN_MACRO_PLAY1 |              |               |
| REC_STOP    | DYN_REC_STOP    |              |               |
| OSL_U_SYM   | OSM(MOD_LSFT)   | &sl U_SYM    |               |

Alexej, 2022-09-27: Note the use of OSM(MOD_LSFT). It is a workaround for the fact that QMK doesn't handle chained OSLs well. It currently requires to patch QMK as shown here https://github.com/qmk/qmk_firmware/issues/6943#issuecomment-1223723779
...
#+NAME: nonkc
| U_ | RGB_ | OUT_ | RESET | S( | C( | SCMD( | LCMD( | DF( | DYN_ | OSL( | OSM( |
