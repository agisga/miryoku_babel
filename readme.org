...
#+NAME: hold-flip
| BOOT  | OSL_U_FUN | OSL_U_SYM | DF(U_BASE) |       |         | DF(U_BASE) | DF(U_EXTRA) | DF(U_TAP) | BOOT     |
| LGUI  | LALT      | LCTL      | LSFT       |       |         | LSFT       | LCTL        | LALT      | LGUI     |
...
| OSL_U_SYM   | OSL(U_SYM)      | &sl U_SYM    |               |
| OSL_U_FUN   | OSL(U_FUN)      | &sl U_FUN    |               |
...
#+NAME: nonkc
| U_ | RGB_ | OUT_ | RESET | S( | C( | SCMD( | LCMD( | DF( | DYN_ | OSL( |
